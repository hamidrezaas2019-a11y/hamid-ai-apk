// ============================================================
// Cloudflare Worker - نسخه نهایی تصحیح‌شده
// ============================================================
...
// ============================================================
// صفحه اصلی کامل (با تمام قابلیت‌ها)
// ============================================================
  ...
  <script>
    ...
    async function sendMessage() {
      ...
      try {
        ...
        var reply = data.choices?.[0]?.message?.content || 'پاسخی دریافت نشد.';
    ...
    async function saveChat(userMessage, assistantReply) {
      var chatId = state.currentChatId || Date.now().toString();
      var chatData = {
        id: chatId,
    ...
    async function loadChat(chatId) {
      try {
        var response = await fetch('/history?id=' + chatId);
        var data = await response.json();
        if (data.chat) {
          state.currentChatId = chatId;
    ...
    function insertCodeTemplate() {
      ...
      var selected = templates[Math.floor(Math.random() * templates.length)];
    ...
    // ============================================================
    // ATTACH FILE - نسخه تصحیح‌شده (رفع خطای کامپایل)
    // ============================================================
    function attachFile() {
      ...
      input.onchange = function(e) {
        ...
        if (file) {
          ...
          reader.onload = function(event) {
            ...
            var ext = file.name.split('.').pop() || 'txt';
            // استفاده از + برای اتصال رشته‌ها به جای بک‌تیک
    ...
    $('temperatureRange').addEventListener('input', function(e) {
      var val = (e.target.value / 100).toFixed(1);
...
async function getChatsFromKV() {
  ...
  return [
    ...
    },
    {
      ...
      title: 'کد پایتون',
      messages: [
        { role: 'user', content: 'یک تابع مرتب‌سازی بنویس' },
